...
public class Board {
    ...
    public Board(Path path){
        ...
        try  {
            ...
            System.out.println("Empty cells: " + (81 - contChar)); //celle vuote
            double percFill = 100 * contChar / 81;
            System.out.println("Fill factor: " + percFill + "%"); //fill factor
            System.out.println("Search space before elimination: " + computeSearchSpace() + " branches"); //dimensione dello spazio delle soluzioni
     ...
     * @return la dimensione dello spazio delle soluzioni del sudoku
     */
    public BigInteger computeSearchSpace() {
        int cellCurrentPosition = 0;
        BigInteger searchSpace = BigInteger.valueOf(1);

        while (cellCurrentPosition < SIZE * SIZE) {
            int row = cellCurrentPosition / SIZE;
            int column = cellCurrentPosition % SIZE;

            cellCurrentPosition++;
            if (!isFilled(row, column))
                searchSpace = searchSpace.multiply(BigInteger.valueOf(getOptionsForCell(row, column).size())); //se la cella è vuota moltiplica se stesso con la dimensione delle opzioni della cella corrente
        }
